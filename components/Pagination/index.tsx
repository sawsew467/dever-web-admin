...
function Pagination({paramID, route, countNumberOfPage, pages, increaseIndex, sliceSetData, data,}: Ipros) {
    ...
    const renderPagination = () => {
        ...
        for(let i = 1; i <= countNumberOfPage; i++) {
            buttons.push(<Link href={`${route}/${i}`} key={i}><button className={`px-[12px] py-[4px] border-r-[1px] ${paramID === i.toString() ? "bg-blue-200 text-blue-600" : ""}`}>{i}</button></Link>);
            pages.push({
                param: i.toString(),
    ...
    return (
        ...
        style={{
            ...
            <div className='border-[1px] rounded-md font-[14px]'>
                <button className={`px-[12px] py-[4px] border-r-[1px] ${paramID === "1" ? "text-slate-300 pointer-events-none" : ""}`}
                ><Link href={`${route}/${paramID === "1" ? "1" : parseInt(paramID) -1}`}>Previous</Link></button>
            
                {
                    renderPagination()
                }
                
                <button className={`px-[12px] py-[4px] ${paramID === countNumberOfPage.toString() ? "pointer-events-none text-slate-300" : ""}`}
                ><Link href={`${route}/${parseInt(paramID) + 1}`}>Next</Link></button>
            </div>
        </div>
    )
}
